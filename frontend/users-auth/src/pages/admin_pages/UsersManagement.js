...
const UserManagement = () => {
  ...
  const confirmChangeUserType = async (userId, newType) => {
    const confirmed = window.confirm(
      `Are you sure you want to change this user's type to ${newType}?`
    );
    if (confirmed) {
  ...
  const changeUserType = async (userId, newType) => {
    try {
      ...
      if (response.ok) {
        setUsers(
          users.map((user) =>
            user._id === userId ? { ...user, type: newType } : user
          )
        );
      } else {
    ...
    <div className="userManagement">
      <div className="pageHeader">
        ...
        <span className="pageDescription">
          The table below reflects all the registered users of Farm To Table.
        </span>
      </div>
      <table>
        <thead>
